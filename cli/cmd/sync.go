...
func init() {
	...
	syncCmd.Flags().StringP("project", "p", "", "Nom du projet (auto-detect sinon)")
...
func syncProject(a *app.App, hubDir string, project *domain.Project) error {
	plan := buildDeployPlan(a, project.Path, project.ID, hubDir, "", "", project.Agents, project.ModelOverrides, project.MCPConfig, project.TeamConfig)
