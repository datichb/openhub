...
- Hub initialized (`oh install`)
- Project registered (`oh init PROJECT_ID`)
...
> **In CI/CD** (`OH_NON_INTERACTIVE=1`): discovery is silently skipped. Deploy continues normally.

### On demand

```bash
oh agent discover MY-PROJECT
```
...
|---------|-------------|---------|
| Agent not detected at deploy | It has the `<!-- generated by openhub` marker | It's a hub agent — it will be regenerated normally |
| Prompt doesn't appear | `OH_NON_INTERACTIVE=1` or no TTY | Run `oh agent discover MY-PROJECT` manually |
