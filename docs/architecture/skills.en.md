...
| `quality/debugger-subagent.md` | debugger (conditional) | **Sub-agent execution path** — loaded by the debugger when `[SKILL:quality/debugger-subagent]` is detected in the prompt (orchestrator injection). Session interruption mechanism at each phase, `## Retour intermédiaire vers orchestrateur` + `## Question pour l'orchestrateur` blocks with `task_id`, `question` tool forbidden. Mirrors the `planner-subagent` pattern. |
