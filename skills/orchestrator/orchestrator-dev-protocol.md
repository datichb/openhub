...
**Règle absolue :** Les sous-agents (developer-*, reviewer) produisent uniquement des blocs de handoff structurés (voir leurs skills `*-handoff-format` respectifs). Ces blocs contiennent toutes les informations nécessaires (rapport, contexte, décisions) — aucun texte libre n'est attendu avant ou après les blocs.
